# INFORME AUDITABLE v4 — Proyecto vinos-carta-app
...
- Separador Markdown roto corregido (cerca de B.2)
- Contador de riesgos actualizado: "6 riesgos principales (R1–R5 y R9)"
- Fecha corregida: quitado "16 de julio" inventado, puesto "same-day revision"
- Inconsistencia 4.4 vs P1 corregida: 800-2.000 → 1.200-4.000 con referencia cruzada

### v3 → v4

- Título de cabecera actualizado de v2 a v4
- Anticipo rellenado: 30% al aceptar, 70% contra entrega (en Opciones A, B y C)
- Decisión E.3 cerrada: Opción B como recomendada (mitiga R1)
- Desglose hosting Opción C rellenado: Cloudflare Pages + Supabase (0–10 €/mes)
- P6 cerrado: inglés descartado para v1, español solo; versión cliente con QR como futura ampliación
- B.2 resuelto: PDF es admin-side (informes de vinos en distribución y restaurantes), no función de PDA
...
| **Decisión** | ✅ **El MVP tiene 4 funciones:** 1) Búsqueda de vinos en la carta del restaurante, 2) Ficha detallada de cada vino (uva, bodega, notas de cata), 3) Recomendaciones por uva parecida, 4) Informe de inventario exportable a PDF (admin-side: listado de vinos por restaurante suscrito, sin cantidades). |
...
| **Decisión** | ✅ **Informe exportable a PDF desde el mini-admin (lado empresa).** El camarero ve su carta en la app (función #1); el PDF es un reporte administrativo que la empresa de vinos genera para consultar vinos en distribución, restaurantes suscritos y sus cartas asociadas. Sin cantidades, sin stock, sin filtros ni agrupaciones. |
| **Justificación** | El coste marginal del PDF es bajo y cubre un caso de uso real de la empresa: tener un informe consolidado de qué vinos tiene cada restaurante en su carta. No es una función para la PDA del camarero. |
...
> **Nota de auditoría (corrección #7) — resuelto v4:** El PDF se usa exclusivamente desde el mini-admin (lado empresa). Sirve para generar informes de vinos en distribución, restaurantes suscritos y sus cartas. No es una función para la PDA del camarero — el camarero ve su carta desde la app (función #1).
...
| **Decisión** | ✅ **Opción B — Proyecto + 3 meses de cambios menores incluidos.** Es la que Edu presentará como recomendada. Resuelve directamente el riesgo R1 (contradicción "sin mantenimiento" vs. cambios futuros): el cliente tiene cobertura pactada desde el minuto uno, sin negociación caso por caso. |
| **Opción A** | Proyecto llave en mano con precio cerrado. Cambios futuros se facturan aparte (por hora o por presupuesto). |
...
| **Forma de pago** | **30% al aceptar la propuesta / firmar el encargo, 70% restante contra entrega.** |
...
**Contexto:** El desarrollo completo (PWA + multi-tenant + offline + 4 funciones + mini-admin) está presupuestado entre 2.500 y 5.000 € para el MVP (según opción), más carga de datos. Como referencia (dato verificado, ver también sección 4.4): una web corporativa simple de freelance cuesta 1.200–4.000 € en el mercado. Esta app es significativamente más compleja (app multi-tenant con lógica de negocio, no una web informativa).
...
### P6. Idioma / clientela extranjera — DECISIÓN CERRADA (v4)

**Decisión de Edu (v4):** ❌ **Inglés descartado para v1.** La app es exclusivamente para camareros en español. No se necesita versión en inglés en el alcance actual.

**Para la propuesta al cliente:** Puede ofrecerse como plus opcional / ampliación futura (fase 2). La idea tiene valor — el camarero sabe español, pero quizás no sabe traducir al inglés en el momento para el cliente.

**Ampliaciones identificadas para futuro:**
- Versión en inglés de la carta (para el camarero o para cliente)
- Versión cliente con QR en mesa (acceso directo del comensal a la carta desde su móvil)

Ambas fuera del alcance de v1 y del presupuesto actual.
